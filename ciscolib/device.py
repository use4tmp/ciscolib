...
class Device(object):
    ...
    def _authenticate(self):
        ...
        if match is None:
            raise AuthenticationError(
                "Unable to get a username or password prompt when trying to authenticate.", text)
            ...
            if self.username is None:
                ...
                if match is None:
                    raise AuthenticationError(
                        "Unexpected text when trying to enter password", text)
                elif match.group().count(b'assword'):
                    self.write(self.password + "\n")

                # Check for an valid login
                idx, match, text = self.expect(
                    ['#', '>', "Login invalid", "Authentication failed"], 2)
                if match is None:
                    raise AuthenticationError(
                        "Unexpected text post-login", text)
                elif b"invalid" in match.group() or b"failed" in match.group():
                    raise AuthenticationError(
                        "Unable to login. Your username or password are incorrect.")
        else:
    ...
    def enable(self, password=None, level=-1):
        ...
        if level < 0:
            ...
            raise CiscoError(
                "Enable level %s is out of the permitted range (0-15)" % (level))
        else:
            self.write("enable %s\n" % (level))
        ...
        if match is None:
            raise CiscoError(
                "I tried to enable, but didn't get a command nor a password prompt")
        else:
            if '#' in text:
        ...
        if match.group() is None:
            raise CiscoError(
                "Unexpected output when trying to enter enable mode", text=None)
            ...
            raise CiscoError(
                "Unexpected output when trying to enter enable mode", text=match.group())

    def expect(self, asearch, ind):

        idx, match, text = self._connection.expect(
            [needle.encode('ascii') for needle in asearch], ind)
    ...
    def get_neighbor(self, port):
        """ Returns a dicts of the switch's neighbor by port: 
            {hostname, ip, local_port, remote_port} """

        re_text = "-+\r?\nDevice ID: (.+)\\b\r?\n.+\s+\r?\n\s*IP address:\s+(\d+\.\d+\.\d+\.\d+)\s*\r?\n.*\r?\nInterface: (.+),.+Port ID.+: (.+)\\b\r?\n"

        neighbor = {}
        match = re.search(re_text, self.cmd(
            'show cdp neighbors ' + port + ' detail'))
        if match is not None:
            neighbor['hostname'], neighbor['ip'], neighbor[
                'local_port'], neighbor['remote_port'] = match.groups()
    ...
    def get_neighbors(self):
        ...
        for neighbor in re.findall(re_text, self.cmd('show cdp neighbors detail')):
            ...
            n_dict['hostname'], n_dict['ip'], n_dict[
                'local_port'], n_dict['remote_port'] = neighbor
    ...
    def get_interface(self, interface):
        ...
        """
        status_re = "(\w{2}\d+(?:/\d+)+)\s+(.+?)\s+(\w+)\s+(\d+|trunk)\s+((?:\d|\w|-)+)\s+((?:\d|\w|-)+)\s+(.+)\r+\n+"

        status_match = re.search(status_re, self.cmd(
            'show interface ' + interface + ' status'))

        port = {}
        if status_match is not None:
            port['name'], port['description'], port['status'], port['vlan'], port[
                'duplex'], port['speed'], port['media'] = status_match.groups()
    ...
    def get_interfaces(self):
        ...
        if port_matches == []:
            raise ModelNotSupported(
                "Unable to parse `show interfaces`", interface_data)

        for match in port_matches:
            port = dict()
            port['name'], port['status'], port['description'] = match

            # Not exactly the most efficient, but it guarantees that we get the
            # right ports
            status_match = re.search(status_re, self.cmd(
                'show interface ' + port['name'] + ' status'))
            if status_match is None:
                ...
                port['vlan'], port['duplex'], port['speed'], port[
                    'media'] = status_match.groups()[3:]

            # port['media'] = port['media'].strip()
            # port['description'] = port['description'].strip()
            port['media'] = port['media']
    ...
    def get_arp_table(self, ip):
        """ 
        Get arp table by ip address, return a dict
        Return fields: {ip, age, mac, interface}
        """
        re_text = 'Internet\s+(?P<ip>\d+\.\d+\.\d+\.\d+)\s+(\d+|-)\s+((?:\d|\w){4}\.(?:\d|\w){4}\.(?:\d|\w){4})\s+ARPA\s+(.+)\r+\n+'
        match = re.search(re_text, self.cmd("show arp | include %s" % ip))

        table = {}
        if match is not None:
            table['ip'], table['age'], table[
                'mac'], table['interface'] = match.groups()
    ...
    def get_mac_table(self, mac):
        """ Returns the mac address table from the device by one mac """
        re_text = '\*?\s+(\d+|All)\s+((?:\d|\w){4}\.(?:\d|\w){4}\.(?:\d|\w){4})\s+(static|dynamic)\s+(?:(?:Yes|No)\s+(?:-|\d+)\s+)?(.+?)\r?\n'

        try:
            data = self.cmd("show mac address-table | include %s" % mac)
        except:
            try:
                data = self.cmd("show mac-address-table | include %s" % mac)
            except:
        ...
        match = re.search(re_text, data, flags=re.I)

        rows = {}
        if match is not None:
            rows['vlan'], rows['mac'], rows[
                'mode'], rows['port'] = match.groups()

        return rows

    def get_mac_tables(self):
